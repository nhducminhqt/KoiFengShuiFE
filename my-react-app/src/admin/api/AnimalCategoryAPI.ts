    ...
    if (response.data.code === 1000) {
      ...
      for (const item of responseData) {
        result.push({
            ...
            item.colors?.map((color: any) => ({
              ...
              destiny: color.destiny
                ? {
                    ...
                    directions:
                      color.destiny.directions?.map((direction: any) => ({
                        id: direction.id,
                        direction: direction.direction,
                      })) || [],
                    numbers:
                      color.destiny.numbers?.map((number: any) => ({
                        id: number.id,
          ...
          animalImages:
            item.animalImages?.map((image: any) => ({
              id: image.id,
...
export async function findByAnimalCategory(
  name: string,
  page: number,
  pageSize: number
): Promise<ResultInterface | null> {
  ...
  try {
    const response = await api.get(
      `/animals/animal-search?search=${name}&page=${page}&size=${pageSize}`
    );
...
export async function findByAnimalDestiny(
  destinyList: string[] = [],
  page: number,
  pageSize: number
): Promise<ResultInterface | null> {
  ...
  try {
    ...
    const response = await api.get(
      `/animals/animal-destiny?destiny=${destinyQuery}&page=${page}&size=${pageSize}`
    );
...
export async function findByAnimalDestinyAndName(
  destinyList: string[] = [],
  name: string,
  page: number,
  pageSize: number
  ...
  try {
    ...
    const response = await api.get(
      `/animals/animal-destiny-name?destiny=${destinyQuery}&name=${name}&page=${page}&size=${pageSize}`
    );
